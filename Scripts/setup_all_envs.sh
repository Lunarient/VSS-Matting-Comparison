#!/bin/bash
# Create and install per-model virtual environments using Python 3.10 from pyenv if available
...
if command -v pyenv >/dev/null 2>&1; then
  if pyenv versions --bare | grep -q '^3\.10'; then
    ...
    echo "⚠️  pyenv found, but no Python 3.10 version installed. Falling back to system python3.10."
...
if [ -z "$PYTHON_BIN" ]; then
  echo "❌ No suitable Python interpreter found. Please install Python 3.10 or pyenv with 3.10."
  exit 1
fi
